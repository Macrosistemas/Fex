...
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Funciones\fn_fecha_comprobante_ajustada.sql"
...
REM 5. Crear Vistas
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\compras_pendientes.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\comprobante_v.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\fex_co_cuentas.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\fex_proveedores.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\fex_sectores_contables.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\fex_sistemas.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\tipo_impresion.sql"
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -i "Vistas\ventas1_pend.sql"


REM Puedes agregar más scripts según corresponda
REM === Verificar objetos instalados ===
sqlcmd -S %SQLSERVER% -d %SQLBASE% -U %SQLUSER% -P %SQLPASS% -Q "WITH ObjetosDeseados AS (SELECT 'Tabla' AS TipoDeseado, name AS nameDeseado FROM (VALUES ('ingreso_comprobantes_compras_cabecera'),('ingreso_comprobantes_compras_items'),('ingreso_comprobantes_compras_asiento'),('ingreso_comprobantes_compras_ib'),('ingreso_comprobantes_codigos_equivalentes'),('script_control')) AS T(name) UNION ALL SELECT 'Funcion', name FROM (VALUES ('f_setea_sector_fex'),('f_sql_comprobante_existente'),('f_sql_limpia_cuit'),('f_sql_numero_asiento'),('f_sql_trae_codigo_cc'),('fn_fecha_comprobante_ajustada')) AS T(name) UNION ALL SELECT 'Procedimiento', name FROM (VALUES ('sp_calcula_iva_compras'),('sp_ingreso_comprobantes_compras'),('sp_inserta_compras_ib'),('sp_inserta_compras1'),('sp_inserta_compras2'),('sp_inserta_compras5'),('sp_inserta_compras7'),('sp_inserta_movi_stock'),('sp_inserta_moviprov1'),('sp_marca_duplicados_comprobantes'),('sp_valida_comprobante_compra')) AS T(name) UNION ALL SELECT 'Vista', name FROM (VALUES ('compras_pendientes'),('comprobante_v'),('fex_co_cuentas'),('fex_proveedores'),('fex_sectores_contables'),('fex_sistemas'),('tipo_impresion'),('ventas1_pend')) AS T(name)) SELECT od.TipoDeseado AS Tipo, od.nameDeseado AS Objeto, CASE WHEN ex.name IS NULL THEN 'FALTANTE' ELSE 'OK' END AS Estado FROM ObjetosDeseados od LEFT JOIN (SELECT name, 'Tabla' AS Tipo FROM sys.tables UNION ALL SELECT name, 'Funcion' FROM sys.objects WHERE type IN ('FN','IF','TF') UNION ALL SELECT name, 'Procedimiento' FROM sys.objects WHERE type = 'P' UNION ALL SELECT name, 'Vista' FROM sys.objects WHERE type = 'V') ex ON od.nameDeseado = ex.name AND od.TipoDeseado = ex.Tipo ORDER BY od.TipoDeseado, od.nameDeseado;"
